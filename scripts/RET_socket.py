...
class Computer_ReceiveMessage_Rpi(threading.Thread,RET_Parameter.RET_Parameter):
    ...
    def run(self):
        ...
        while RET_config.stop_thread == False:
            ...
            try:
                self.parameter.list_msg_change_state = received_message.split(";")
                time_to_process = self.parameter.list_msg_change_state[0]
                if self.parameter.list_msg_change_state[2] == "pressed":
                    self.parameter.time_Btn_pressed = datetime.datetime.strptime(time_to_process, '%Y-%m-%d %H:%M:%S.%f')
                    self.parameter.end_effector_position_received_socket_message_pressed = [self.parameter.time_Btn_pressed]
                if self.parameter.list_msg_change_state[2] == "unpressed":
                    self.parameter.time_Btn_unpressed = datetime.datetime.strptime(time_to_process, '%Y-%m-%d %H:%M:%S.%f')
                    self.parameter.end_effector_position_received_socket_message_unpressed = [self.parameter.time_Btn_unpressed]
            except:
                pass
